...
async def test_authentication():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(f"{BASE_URL}/api/auth/login", json=TEST_CREDENTIALS)
        ...
        response = await client.get(f"{BASE_URL}/api/protected", headers=headers)
        assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
...
async def test_adapter_creation(token: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{BASE_URL}/api/adapters/postgres", headers=headers, json=POSTGRES_CONFIG
        )
        ...
        response = await client.post(
            f"{BASE_URL}/api/adapters/rest_api", headers=headers, json=REST_API_CONFIG
        )
...
async def test_adapter_execution(token: str, instance_id: str):
    ...
    async with httpx.AsyncClient() as client:
        # Execute query
        response = await client.post(
            f"{BASE_URL}/api/adapters/{instance_id}/execute", headers=headers, json=TEST_QUERY
        )
        ...
        if response.status_code == 500:
            ...
            logger.warning(
                "This is expected in a test environment without actual database connections"
            )
