...
with open(old_repositories_json_path, encoding="utf-8") as of:
    ...
    for repository in old_data["repositories"]:
        ...
        if user_match:
            api_url = "https://api.github.com/users/%s/repos?per_page=100&%s" % (
                user_match.group(1),
                client_auth,
            )
    ...
    for repository in repositories_to_process:
        ...
        if repo_match:
            ...
            if name in [
                ...
                "whocalled",
                "jsonlint",
                "symfonytools-for-sublimetext-2",
                "html-compress-and-replace",
                "sublime-aml",
                "quick-rails",
                "quickref",
                "smartmovetotheeol",
                "sublime-http-response-headers-snippets",
                "sublimesourcetree",
                "zap-gremlins",
                "andrew",
                "bootstrap-jade",
            ...
            if repo_match.group(1).lower() == "github.com":
                release_url = "https://github.com/%s/%s/tree/%s" % (
                    repo_match.group(2),
                    repo_match.group(3),
                    branch,
                )
            else:
                release_url = "https://bitbucket.org/%s/%s/src/%s" % (
                    repo_match.group(2),
                    repo_match.group(3),
                    branch,
                )
...
for letter in names:
    ...
    with open(include_path, "w", encoding="utf-8") as f:
        data = OrderedDict([("schema_version", "2.0"), ("packages", [])])
