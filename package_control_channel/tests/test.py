...
class TestContainer:
            ...
            if k == "details":
                ...
                for prev_name in v:
                    if prev_name in self.previous_package_names:
                        ...
                        self.previous_package_names[prev_name] = (prev_name, include, name)
        ...
        for k, v in data.items():
            ...
            if k == "url":
                ...
                if (
                    ...
                    or ("windows-x32" in v and "windows-x64" in v)
                    or ("linux-x32" in v and "linux-x64" in v)
                ):
    ...
    def enforce_key_types_map(self, k, v, key_types_map):
        ...
        if isinstance(v, list) and key_types_map[k] != list and len(key_types_map[k]) == 2:
                ...
                if "releases" in package:
                    for release in package["releases"]:
                        (
                            ...
                            ("%s (%s)" % (package_name, path), release, False, False),
                        )
        if "includes" in data:
...
class DefaultRepositoryTests(TestContainer, unittest.TestCase):
    ...
    def test_repository_keys(self):
        keys = sorted(self.j.keys())
        self.assertEqual(keys, ["dependencies", "includes", "packages", "schema_version"])
                ...
                for dependency in data["dependencies"]:
                    ...
                    if "releases" in dependency:
                        for release in dependency["releases"]:
                            (
                                yield cls._test_release,
                                ("%s (%s)" % (dependency_name, include), release, True),
                            )
