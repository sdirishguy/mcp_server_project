...
class AuthenticationResult(BaseModel):
    ...
    roles: Optional[List[str]] = Field(None, description="Roles assigned to the user")
    permissions: Optional[List[str]] = Field(None, description="Permissions granted to the user")
...
class InMemoryAuthProvider(AuthenticationProvider):
    ...
    async def refresh_token(self, refresh_token: str) -> AuthenticationResult:
        ...
        Args:
            refresh_token: The refresh token (same as the original token in this implementation)
            
        Returns:
            AuthenticationResult: Result with a new token if successful
        """
        # Validate the refresh token (which is the same as the access token in this simple implementation)
