...
from environment variables and `.env` files. It consolidates variables so that
...
class Settings(BaseSettings):
    ...
    JWT_EXPIRY_MINUTES: int = Field(default=60, description="Minutes before issued JWT tokens expire")

    # Test bypass (CI only; empty = disabled)
...
try:
    ...
    logger.error("Could not create or access MCP base working directory %s: %s", _base_path, exc)

# In production, disallow arbitrary shell commands unless explicitly enabled
